...
export default function Home() {
  ...
  const handleRegion = (regionz)=>{
    if( !regionz ==  region && region == ""){

      const regionData = data.filter((items)=>{
        return items.region === regionz;
        
      })
      setfiltered(regionData);
      setRegion(regionz);
    }
    else(!region == regionz);{
      const regionData = data.filter((items)=>{
  ...
  return (
    ...
    <Layout>
      <div className={`${isActive ? "Dark-mode" : ""} lg:items-center lg:justify-center flex flex-col text-skin-base font-Nunito  gap-8 bg-skin-fill  justify-between pl-8 pr-8`}>
        <div className="lg:flex lg:items-center lg:pl-10 lg:pr-10 lg:w-full lg:justify-between lg:mb-32">

          <div className=' '>
            <form>
              <div className='relative lg:w-96 mt-10 rounded-lg shadow-lg drop-shadow-xl flex justify-center w-full h-14 '>
            ...
            <div className='lg:w-52 w-52 mt-5 lg:absolute lg:right-16 lg:top-32 relative '>
                ...
                <div className=' bg-skin-fill  rounded-t-lg overflow-auto flex-wrap left-16 w-full h-44 shadow-2xl'>
                  <ul className=" flex flex-col ml-5 mt-8  pb-8 text-lg gap-2">
                    {data.filter((item, index, self)=>(self.slice(0, index).findIndex((t) => _.isEqual(t.region, item.region)) === -1 )).map((names)=>{
                      return(
                        <Link
                          onClick={()=>handleRegion(names.region)}  
                          href="" 
        ...
        <div className="grids w-full gap-10 flex flex-col items">
          {filtered.map((items)=>{
            return (
